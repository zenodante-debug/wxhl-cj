...
/**
 * 位阶修正值系数（用户规则 6-2）：一阶1 / 二阶2 / 三阶4 / 四阶7 / 五阶11。下标即 `归一位阶` 的返回值
 */
const 位阶修正系数 = [1, 2, 4, 7, 11];

// 阶位写法归一**不再由本模块自己做** —— 统一走 `dice.ts` 的 `归一位阶`
// （汉字 / `N阶` / 裸数字 `N` / 前导「第」/ 全角 / 繁体「階」…都认）。
// 归一后仍认不出的（`'无'`、`'六阶'`、拼错、字段缺失）＝**阶位不可识别**,
 ...
 * （AI 漏写被 `prefault('无')` 补上、拼错、或干脆缺失）时
 ...
 * - 「修正值」=（实际属性值 − 5）× 位阶修正系数
 * - **阶位先经 `dice.ts:归一位阶` 归一**（`'1阶'` / `'第3阶'` / `'３阶'` → 一阶..五阶的 0..4）;
 *   归一后仍不可识别时,
...
function 算衍生(输入: 衍生输入): EnemyDerivedStats {
  const 位阶 = 归一位阶(输入.阶位);
  const 系数 = 位阶 === undefined ? undefined : 位阶修正系数[位阶];
 ...
 *
 * 回退模式下, 若**阶位不可识别**（`'无'` / `'六阶'` 之类拼错 / 字段缺失）, 自算的

...
/**
 * 衍生属性的 7 个「额外加成」白名单键 —— **从 schema 派生**, 不再手工双写。
 *
 * 手工维护两份 7 键清单只靠注释维系: 将来给 schema 加了第 8 个键而漏改清单,
 * `mapEnemyToVariables` 的白名单重建会**静默丢弃**该字段（变量少一项、前端加不上）, 且测试不会红。
 * 派生自 `Object.keys(schema.shape)` 后, 加键自动生效, 漂移不可能发生。
 */
const 衍生额外加成键 = Object.keys(衍生额外加成Schema.shape) as (keyof z.output<typeof 衍生额外加成Schema>)[];
...
/**
 * 回退（未代算）模式下渲染一个**自算**数值。
 * 阶位可识别时它是有限数, 原样打印; 阶位不可识别时 `算衍生` 给的是 `undefined`
 * → 印 `—`。类型上只有这两种可能, 故不做 `Number.isFinite` 兜底。
 */
function 印自算(值: number | undefined): string {
  return 值 === undefined ? '—' : String(值);
}
...
/**
 * 阶位的别名写法 → 规范写法。与 `store.ts` 的 `阶位归一` 是**同一套口径**
 * （那里同时列了「一阶」与「1阶」两种写法, 说明 AI 真的会写后者）, 此处在同一套
 * 口径上再加裸数字写法 `1`~`5`。
 *
 * 归一后仍查不到系数的（`'无'`、`'六阶'`、拼错、字段缺失）＝**阶位不可识别**,
 * 此时派生数值一律印 `—` —— 绝不 `?? 1` 静默按一阶算（五阶真实系数是 11, 按 1 算
 * 会让 最大MP 差 11 倍, 却给出一个"像真的"的数）。
 */
...
/** 把阶位写法归一到「一阶」~「五阶」; 无法识别时返回 undefined（**不是**退回一阶） */
function 归一位阶(阶位: unknown): string | undefined {
  return typeof 阶位 === 'string' ? 阶位别名[阶位] : undefined;
}
 ...
 * 面板用的衍生属性计算结果。
 *
 * 除 `负重上限` 外的六项都依赖**位阶修正系数**（用户规则 6-2）: 阶位不可识别
 * （AI 漏写被 `prefault('无')` 补上、拼写成 `'1阶'` 之类未被归一、或干脆缺失）时
 * 它们是 `undefined`, 由面板印 `—` —— 宁可印 `—` 也不静默按一阶系数算出一个
 * "看着像真的"的数（五阶真实系数 11, 按 1 算会让 最大MP 差 11 倍）。
 *
 * `负重上限 = STR实际值 × 5 + 负重额外加成` **只依赖 STR、不依赖阶位**,
 * 故任何阶位下都算得出, 恒为 number。
 */
export interface EnemyDerivedStats {
  最大HP: number | undefined;
  最大MP: number | undefined;
  最大耐力: number | undefined;
  防御: number | undefined;
  闪避值: number | undefined;
  移动距离: number | undefined;
  负重上限: number;
}
 ...
 *
 * 判据取**四信号**: `衍生属性` 的 `HP_最大 / 闪避值 / MP_最大 / 负重_上限` 任一 > 0。
 * 这四项由**同一支前端脚本一次算完**, 同生共死; 而 MVU 的 prefault 只会把缺失键补 0,
 * 四维又都 ≥ 0, 故 `0 > 0` 恒假 —— **加这种同源的 `> 0` 信号是单调安全的**: 只会救回漏网,
 * 不会把「未代算」误判成「已代算」。
 *
 * 为什么不能只看 `HP_最大 > 0`: 「前端代算出的 HP_最大 恒为正」是**假的不变量**。
 * 一阶时 HP_最大 =（CON修正+5）× HP系数 = CON × HP系数（额外加成置 0 时）,
 * 而 `四维Schema` 只要求 `min(0)`, **CON = 0 是合法值**（构筑设计思路还明确鼓励
 * 「完全放弃防御、容错率为零」的极端 dump）: 二阶 CON≤2、三阶 CON≤3、四阶/五阶 CON≤4 时同样 ≤ 0。
 * 单信号会把这批合法角色误判成「未代算」→ 面板静默走回退 → 四维取基础、装备加成不可见,
 * 正是本轮要消灭的那个偏低 bug 原样复活且毫无报错。
 *
 * 四信号联合的覆盖面 —— **只在下面三条隐含前提全都成立时才算密封**。三条 schema 都不强制它们,
 * 任何一条被破坏都会造成假阴性: 判据误报「未代算」→ **整面板静默走回退** → 四维取基础、
 * 装备加成不可见, 正是本轮要消灭的那个偏低 bug 原样复活且毫无报错。
 *
 *   前提① **四维真的按 §6-1 的总和分配**（总和 = `30 + 等级 + 类型系数 × 13 × 阶位数²`, 恒 ≥ 31）。
 *     总和下限是整个密封论证的支点, 但 `敌人Schema` 并不强制它: `阶位` 只是 `prefault('无')`、
 *     `等级` 只有 `min(1)`, 所以 `"五阶 Lv.1"`（高阶位系数 11 配极低总和）也能过校验。
 *   前提② **7 个额外加成 ≥ 0**。`衍生额外加成Schema` 每个键都是 `z.coerce.number().prefault(0)`,
 *     **没有 `min`**, 而 §6-1 的总和公式完全不约束额外加成 —— 这是**唯一能真正破密封**的东西。
 *     反例（合法）: 一阶 · 杂兵 · Lv.1, 四维 `{STR:0, AGI:39, CON:0, PER:0}`（总和 38.8 ≈ 39 ✅）,
 *     `闪避额外加成 = −27`, 其余额外加成 0, 无装备 →
 *     `HP_最大 = ((0−5)+5)×8 = 0`、`MP_最大 = (0−5)×1×10 = −50`、`负重_上限 = 0×5 = 0`、
 *     `闪避值 = 10 + ⌊(39−5)×1×0.5⌋ + (−27) = 10 + 17 − 27 = 0`
 *     —— **四信号全 ≤ 0, 但前端确实跑完了**。
 *   前提③ **Σ装备闪避 ≥ 0**。步骤二明文允许重装/极重的 `装备闪避` 写负数（见上面的防闪公式）。
 *
 * 三条前提成立时: 四维都 ≥ 0 且不可能全 0（前提①）, 被 dump 空的维度必然把点数分到别处,
 * 而 CON / STR / PER / AGI 分别由 `HP_最大` / `负重_上限` / `MP_最大` / `闪避值` 兜住。
 * `闪避值` 是最硬的兜底, 但**它并非无条件 > 0**:
 *   · 一阶 `AGI = 0` 时 `闪避值 = 10 + ⌊−5×1×0.5⌋ = 10 − 3 = 7`, 这只在 `Σ装备闪避 = 0`
 *     且 `闪避额外加成 = 0` 时成立; 一旦重装/极重装甲堆叠就会被压到 ≤ 0
 *     （一阶 4 个防具槽全极重 → `Σ装备闪避 = 4×⌊7×0.6²×1×(−0.5)⌋ = 4×(−2) = −8`,
 *     此时一阶 AGI=0 的 `闪避值 = 10 + ⌊−2.5⌋ − 8 = −1`）;
 *   · 二阶 `AGI = 0` 时 `闪避值 = 10 + ⌊−5×2×0.5⌋ = 5`, 同样以额外/装备为 0 为条件。
 * 换句话说, 只有极重装甲堆叠 + 负额外加成这种极端叠加才可能把 `闪避值` 压到 ≤ 0,
 * 那种情况下要靠「补满总和必须把点数放进 STR/CON/PER」来兜。
 * ⚠️ 「总和下限 ⇒ 至少一维 ≥ 8」本身并**不**保证那一维是 STR 或 PER,
 * 所以不能只靠 `负重_上限` / `MP_最大` 两项, 必须四条一起看。
 *
 * 也不用 `属性.实际` 判定: prefault 会把它补成 {STR:0,...} 这种"看着存在其实是空的"值;
 * 且本仓库 `PvPSaveSchema` 的 `实际` 是 parse 期 `.transform()` 就算出来的,
 * 脚本跑之前它就已经非零, **不能当判据信号**。
 */
export function 前端已代算(实体: 回读实体 | undefined | null): boolean {
  const 衍生 = (实体 as any)?.衍生属性;
  return 取数(衍生?.HP_最大, 0) > 0
    || 取数(衍生?.闪避值, 0) > 0
    || 取数(衍生?.MP_最大, 0) > 0
    || 取数(衍生?.负重_上限, 0) > 0;
}
 ...
 * - 「修正值」=（实际属性值 − 5）× 位阶修正系数
 * - **阶位先经 `归一位阶` 归一**（`'1阶'` → `'一阶'`）; 归一后仍不可识别时,
 *   六项依赖系数的结果一律 `undefined`（面板印 `—`）, 只有 `负重上限` 照常算出
 * - `负重上限` 用的是 **STR 实际值**（不是 STR 修正值）
 * - `Σ装备防御`/`Σ装备闪避` 为七槽求和（AI 已按步骤二公式算好绝对值, 缺槽按 0 计）
 */
function 算衍生(输入: 衍生输入): EnemyDerivedStats {
  const 归一 = 归一位阶(输入.阶位);
  const 系数 = 归一 === undefined ? undefined : 位阶修正系数[归一];
  ...
  // 阶位不可识别 → undefined, 调用方印 `—`（绝不退回一阶系数）
  const 修正 = (实际值: number): number | undefined => (系数 === undefined ? undefined : (实际值 - 5) * 系数);

  const CON修正 = 修正(基础.CON ?? 0);
  const AGI修正 = 修正(基础.AGI ?? 0);
  const PER修正 = 修正(基础.PER ?? 0);
  // 负重上限用的是 STR 实际值, 不是 STR 修正值; 它不依赖阶位, 故不可识别时仍算得出
  const STR实际值 = 基础.STR ?? 0;
  const 防闪 = 汇总装备防闪(输入.装备);

  return {
    最大HP: CON修正 === undefined ? undefined : (CON修正 + 5) * (HP系数[输入.类型 as EnemyKind] ?? 8) + 取额外('HP额外加成'),
    最大MP: PER修正 === undefined ? undefined : PER修正 * 10 + 取额外('MP额外加成'),
    最大耐力: CON修正 === undefined ? undefined : (CON修正 + 5) * 10 + 取额外('耐力额外加成'),
    防御: CON修正 === undefined ? undefined : Math.floor((CON修正 + 5) * 0.2) + 防闪.防御 + 取额外('防御额外加成'),
    闪避值: AGI修正 === undefined ? undefined : 10 + Math.floor(AGI修正 * 0.5) + 防闪.闪避 + 取额外('闪避额外加成'),
    移动距离: AGI修正 === undefined ? undefined : 5 + AGI修正 + 取额外('移动距离额外加成'),
 ...
 *
 * 回退模式下, 若**阶位不可识别**（`'无'` / 拼写不在别名表里 / 字段缺失）, 自算的
 * `最大HP / 防御 / 闪避值` 等六项是 `undefined` → 印 `—`（`[生命|` 印 `—/—`、
 * `[防御|` 印 `【防御】— | 【闪避】—`）。**这是刻意的**: 阶位不可识别却静默按一阶系数
 * 算, 比不显示更坏。但 `[属性|` 行照常输出 —— 等级/阶位/四维是实打实的值,
 * 阶位**就显示那个可疑的原值**, 让人看得见问题出在哪。
 *
...
export function assembleEnemyPanelFromEntity(名称: string, 实体: 回读实体, 威胁?: string): string {
  ...
  L.push(`[类型|${类型}]`);
  L.push(`[外观|${实体?.外貌 ?? '无'}]`);
  // 前端首次 insert 时会把当前值 = 最大值, 故正常路径下 HP_当前 必然在;
  // 但「战斗后再次生成面板」时两者不等, 必须老实读 HP_当前 —— 缺了就印 `—`,
  // **绝不能拿 HP_最大 顶上**（那会把一个受伤的敌人印成满血, 正是本轮要消灭的"圆上"）
  const 生命 = 已代算
    ? `${印数(衍生属性?.HP_当前)}/${印数(衍生属性?.HP_最大)}`
    : `${印自算(衍生.最大HP)}/${印自算(衍生.最大HP)}`;
  L.push(`[生命|${生命}]`);
  L.push(`[威胁|${威胁文本}]`);
  ...
  L.push(`[防御|${已代算 ? `【防御】${印数(衍生属性?.防御)} | 【闪避】${印数(衍生属性?.闪避值)}` : `【防御】${印自算(衍生.防御)} | 【闪避】${印自算(衍生.闪避值)}`}]`);

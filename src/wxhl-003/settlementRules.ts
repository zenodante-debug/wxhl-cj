...
/** 任务名 + 变量里的奖励原文；变量里没有该键名时如实标注, 不假装它没有奖励 */
function 条目与奖励(名: string, 表: Record<string, string>): string {
  const 原文 = 表[名];
  return 原文 === undefined ? `${名}（变量中未找到）` : `${名}（${原文}）`;
}
...
export function assembleSettlementPanel(
  ...
  const 表 = 快照.任务奖励 ?? {};
  const 掉落清单 = ai.掉落物品.length
    ? ai.掉落物品.map(d => `${d.名称}（${d.品质}｜${d.属性}｜${d.效果}）×${d.数量}`).join('；')
    : '无';
  const 已达成 = ai.达成的成就.length ? ai.达成的成就.map(名 => 条目与奖励(名, 表)).join('；') : '无';
  const 隐藏公示 = ai.完成的隐藏任务.length ? ai.完成的隐藏任务.map(名 => 条目与奖励(名, 表)).join('；') : '无';
  // 快照里只有「奖励」原文, 没有成就清单与达成条件 —— 宁可明说缺什么, 也不要编一份「未达成」
  const 未达成 = '（本模块不持有成就清单与达成条件; 未达成的成就与条件见副本任务面板）';
  const 现实时间 = c.新现实日期
    ? `${c.新现实日期} ${快照.当前现实时间}`
  ...
  L.push(`## 当前PEXP: ${快照.当前PEXP}`);
  L.push(`## 职业进度: ${快照.职业等级}/${快照.PEXP_升级所需}`);
  L.push(`## 称号获得: ${ai.称号?.名称 ?? ''}`);
  L.push(`## 称号效果: ${称号效果文本(ai.称号?.效果)}`);
  L.push('## 称号选择: 待聊天中决定');
  L.push(`## 掉落清单: ${掉落清单}`);
  L.push(`## 副本成就已达成: ${已达成}`);
  L.push(`## 副本成就未达成: ${未达成}`);
...
export function buildSettlementWrites(
  ...
  记(['头部', '称号', '备用称号（只记录不生效）'], ai.称号 ?? { 名称: '无', 效果: {} });
  ...
  // ── 掉落（规则第八步）────────────────────────────────────────────────
  for (const d of ai.掉落物品) {
    记(['背包', d.名称, '数量'], d.数量);
    记(['背包', d.名称, '描述'], `${d.品质}｜${d.属性}｜${d.效果}`);
  }

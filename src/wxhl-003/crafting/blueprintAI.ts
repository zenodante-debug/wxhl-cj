...
export function sanitizeDesign(
  ...
  // 两边都空必须拒（终审 I1）：空名会一路变成 blueprintItemName('') = 「图纸·」→ 玩家花 UP 买到一张
  // 无名图纸 → 上传后 配方库[''] → 制作出**空名背包条目**（主卡背包的空键），无法上架/识别、可无限复制。
  // 设计表单已不强制填名（交给 AI 起名），这个兜底只能落在写入侧——与「宁可拒掉这次定制、只亏 token」同口径。
  if (名称 === '') return { ok: false, reasons: [...reasons, '成品名为空：玩家未填且 AI 未起名——请填写成品名称'] };
  const 配方输入 = {
    名称,
    来源: '图纸',
    行业: 目标.行业,

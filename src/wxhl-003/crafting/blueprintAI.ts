...
export const DESIGN_SCHEMA = {
  ...
  value: {
    ...
    properties: {
      ...
      道具固定值: { type: 'number', description: '道具必填：非负数值基准（恢复量/骰数），超过品质上限会被系统钳制' },
...
道具固定值：非负数字。恢复HP/恢复MP＝每次恢复量基准；爆炸物＝每阶骰数基准；
...
export function sanitizeDesign(
  ...
  if (目标.成品类型 === '道具') {
    ...
    // 关联属性 只影响 buildGoods 的属性修正项，不值得为它拒掉整张图纸：非法值落回 PER 并留痕
    const 关联原文 = o.关联属性;
    if (关联原文 !== undefined && 关联原文 !== 'PER' && 关联原文 !== 'CON') {

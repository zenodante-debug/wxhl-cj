...
/** 道具的功能类型（决定成品数值口径：恢复量 / 骰数 / 状态） */
export const 道具类型列表 = ['恢复HP', '恢复MP', '状态', '弹药', '餐食', '爆炸物', '陷阱', '其他'] as const;
export type 道具类型 = (typeof 道具类型列表)[number];
...
export const 配方Schema = z.object({
  ...
  /** 道具数值（替代旧的按名查表） */
  道具类型: z.enum(道具类型列表).prefault('其他'),
...
// ---- 内置标准道具配方（道具；照抄世界书物价表商品）----
const goods = (
  名称: string, 行业: 行业, 品质: '白色' | '蓝色',
  材料: { 类别: MaterialCategory; 数量: number; 核心?: boolean }[],
  批量上限: number,
  数值: { 道具类型: 道具类型; 道具固定值: number; 关联属性: 'PER' | 'CON' },
): 配方 =>
  配方Schema.parse({
    名称, 来源: '标准', 行业, 成品类型: '道具', 品质,
    材料, 技能要求: { 分类: '基础', 等级: 品质 === '白色' ? 1 : 3 }, 批量上限,
    ...数值,
  });

export const STANDARD_GOODS_RECIPES: 配方[] = [
  goods('基础治疗药剂', '炼金', '白色', [{ 类别: '草药', 数量: 2, 核心: true }], 5, { 道具类型: '恢复HP', 道具固定值: 20, 关联属性: 'PER' }),
  goods('强效治疗药剂', '炼金', '蓝色', [{ 类别: '草药', 数量: 3, 核心: true }], 5, { 道具类型: '恢复HP', 道具固定值: 50, 关联属性: 'PER' }),
  goods('基础精神药剂', '炼金', '白色', [{ 类别: '草药', 数量: 2, 核心: true }], 5, { 道具类型: '恢复MP', 道具固定值: 25, 关联属性: 'PER' }),
  goods('强效精神药剂', '炼金', '蓝色', [{ 类别: '草药', 数量: 3, 核心: true }], 5, { 道具类型: '恢复MP', 道具固定值: 55, 关联属性: 'PER' }),
  goods('净化药剂', '炼金', '蓝色', [{ 类别: '草药', 数量: 2, 核心: true }, { 类别: '矿石', 数量: 1 }], 5, { 道具类型: '状态', 道具固定值: 0, 关联属性: 'PER' }),
  goods('万能解毒剂', '炼金', '蓝色', [{ 类别: '草药', 数量: 2, 核心: true }, { 类别: '怪物素材', 数量: 1 }], 5, { 道具类型: '状态', 道具固定值: 0, 关联属性: 'PER' }),
  goods('普通弹药20发', '工程', '白色', [{ 类别: '金属', 数量: 1, 核心: true }, { 类别: '火药', 数量: 1 }], 5, { 道具类型: '弹药', 道具固定值: 0, 关联属性: 'PER' }),
  goods('穿甲弹药20发', '工程', '蓝色', [{ 类别: '金属', 数量: 2, 核心: true }, { 类别: '火药', 数量: 1 }], 5, { 道具类型: '弹药', 道具固定值: 0, 关联属性: 'PER' }),
  goods('元素弹药20发', '工程', '蓝色', [{ 类别: '金属', 数量: 1, 核心: true }, { 类别: '能量', 数量: 1 }, { 类别: '火药', 数量: 1 }], 5, { 道具类型: '弹药', 道具固定值: 0, 关联属性: 'PER' }),
  goods('战时干粮', '烹饪', '白色', [{ 类别: '食材', 数量: 2, 核心: true }], 3, { 道具类型: '餐食', 道具固定值: 0, 关联属性: 'CON' }),
  goods('增益餐食', '烹饪', '蓝色', [{ 类别: '食材', 数量: 3, 核心: true }], 3, { 道具类型: '餐食', 道具固定值: 0, 关联属性: 'CON' }),
  goods('制式爆炸物', '工程', '白色', [{ 类别: '火药', 数量: 2, 核心: true }, { 类别: '金属', 数量: 1 }], 3, { 道具类型: '爆炸物', 道具固定值: 0, 关联属性: 'PER' }),
  goods('烈性爆炸物', '工程', '蓝色', [{ 类别: '火药', 数量: 3, 核心: true }, { 类别: '能量', 数量: 1 }], 3, { 道具类型: '爆炸物', 道具固定值: 0, 关联属性: 'PER' }),
  goods('绊线陷阱', '工程', '白色', [{ 类别: '金属', 数量: 1, 核心: true }, { 类别: '布料皮革', 数量: 1 }], 3, { 道具类型: '陷阱', 道具固定值: 0, 关联属性: 'PER' }),
];

 ...
 *  故选不出错值。三张表**两两不相交**（武器名 vs 光谱 vs 空集），故「属于哪个子类」可由值本身判定。 */
function 模板选项(子类: string): readonly string[] {
  return 子类 === '武器' ? weaponTypes : 子类 === '防具' ? armorTypes : [];
}

/** 数值参照是**子类相关**的（武器=武器名 / 防具=光谱 / 饰品=无此项），但 `定制.数值参照` 只有一个字段，
 *  换子类后旧值必然落空：控件会渲染成「无匹配选项」，而 提交定制 的 数值倾向 还会把
 *  「数值参照希望用『巨剑』」写进一张**防具**图纸的设计要求里 —— 模型自己的 数值说明
 *  （blueprintAI 的 防具分支）恰恰叫它忽略跨子类的模板要求，于是这次生成白跑（不扣 UP，但要等）。
 *  故与 定制.阶位 同用 watchEffect 把不变量按死：值不属于当前子类的模板集就清空
 *  （留空 = 由 AI 选，是合法态；饰品无此项，故任何残留值都会被清掉）。 */
watchEffect(() => {
  if (定制.数值参照 !== '' && !模板选项(定制.子类).includes(定制.数值参照)) {
    定制.数值参照 = '';
  }
});

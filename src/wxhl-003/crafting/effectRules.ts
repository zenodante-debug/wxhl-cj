...
//   - 无敌 → 看「持续多久」，只认时长/次数类（回合|持续|每场|次数），条件类不豁免
// ================================================================
...
/**
 * 无敌类豁免短语：**只认时长/次数类**——世界书对无敌的约束是「限定明确回合数且单次≤2回合」，
 * 它要的是时长上限，不是触发时机。故条件类八词（低于|不足|以上|以下|概率|几率|时|则）**不**豁免无敌：
 * 否则 `{描述:'无敌', 触发条件:'受到攻击时'}` 会放行一个无时限的无敌，是违禁门缺口。
 */
const 无敌豁免词 = '回合|持续|每场|次数';

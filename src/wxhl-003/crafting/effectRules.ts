...
// 会被消耗字段洗白。故豁免词表只收条件类短语（低于|不足|以上|以下|概率|几率|时|则）。
// ================================================================
...
/** 违禁项：命中即整体拒绝（不钳制）。硬子句不受条件词影响，先命中先拒 */
export const FORBIDDEN_PATTERNS: readonly { 名: string; test: RegExp }[] = [
  // 硬子句「无条件…即死/秒杀」；守卫式 = 全串无条件字样却出现 即死/秒杀
  { 名: '无条件即死', test: new RegExp(`无条件[\\s\\S]*(?:即死|秒杀)|${无条件守卫}(?:即死|秒杀)`) },
  // 硬子句「永久/无限/绝对无敌」；守卫式 = 全串无回合/持续等限次字样却出现 无敌
  // （无敌保留自己的豁免词表：消耗类不得豁免的是即死/必中类禁令；无敌的平衡杆是回合/持续时限）
  { 名: '永久无敌', test: /(?:永久|无限|绝对)\s*无敌|^(?![\s\S]*(?:回合|次数|消耗|持续))[\s\S]*无敌/ },

...
//     「核心弱点必中」「攻击要害，必中」会漏网。
// 违禁扫描只看「描述」字段（与任务书一致）：条件/消耗必须写在描述里才免除守卫式，
// 否则「描述=即死 + 消耗=每场1次」会被字段里的条件词洗白而放行。
// ================================================================
...
export const FORBIDDEN_PATTERNS: readonly { 名: string; test: RegExp }[] = [
  // 硬子句「无条件…即死/秒杀」；否则 = 全串找不到任何条件/消耗字样却出现 即死/秒杀
  {
    名: '无条件即死',
    test: /无条件[\s\S]*(?:即死|秒杀)|^(?![\s\S]*(?:(?<!无)条件|消耗|每场|低于|概率|几率))[\s\S]*(?:即死|秒杀)/,
  },
  // 硬子句「永久/无限/绝对无敌」；否则 = 全串无回合/次数/消耗/持续字样却出现 无敌
  { 名: '永久无敌', test: /(?:永久|无限|绝对)\s*无敌|^(?![\s\S]*(?:回合|次数|消耗|持续))[\s\S]*无敌/ },
  { 名: '无限资源/锁血', test: /无限(资源|弹药|MP|HP)|锁血|血量锁定/ },
  // 双向匹配（必中 在前或在后都算），并叠加「全串无条件字样」守卫
  {
    名: '无条件必中核心弱点',
    test: /^(?![\s\S]*(?:(?<!无)条件|消耗|每场|概率|几率))[\s\S]*(?:必中[\s\S]*(?:核心|弱点|要害)|(?:核心|弱点|要害)[\s\S]*必中)/,
  },
];

...
describe('opFeeFor · 费用曲线（蓝武一阶基准 100 的工作表逐行核对）', () => {
  ...
  it('一阶→超脱：超模费 45×50=2250 RP + 超脱上架费 20 RP = 2270 RP；UP = 基准×500×45 + 超脱基准价×50%', () => {
    const f = opFeeFor(0, 5, base)!;
    expect(f.sum).toBe(45);
    expect(f.rp).toBe(2250 + 20); // 超模 2250 + 超脱上架费 20
    // 超脱基准价 = 一阶基准 × 500 = 50,000；×50% = 25,000
    expect(f.up).toBe(100 * 500 * 45 + Math.floor(100 * 500 * 0.5)); // 2,250,000 + 25,000
  });
  it('用户原例：名义三阶→真实超脱 = 超模 50×(4+7+11+20)+超脱费20 = 2120 RP；UP = 400×500×42 + 超脱基准价×50%', () => {
    const f = opFeeFor(2, 5, 400)!; // 金武
    expect(f.sum).toBe(4 + 7 + 11 + 20);
    expect(f.rp).toBe(50 * 42 + 20); // 2100 + 20
    // 超脱基准价 = 一阶基准(400) × 500 = 200,000；×50% = 100,000
    expect(f.up).toBe(400 * 500 * 42 + Math.floor(400 * 500 * 0.5)); // 8,400,000 + 100,000
  });
  it('非超脱不收超脱上架费（一阶→五阶）', () => {
